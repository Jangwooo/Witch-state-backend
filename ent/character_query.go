...
func (cq *CharacterQuery) QueryProducts() *ProductQuery {
...
func (cq *CharacterQuery) QueryRecords() *RecordQuery {
	query := (&RecordClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CharacterQuery) WithProducts(opts ...func(*ProductQuery)) *CharacterQuery {
	query := (&ProductClient{config: cq.config}).Query()
...
func (cq *CharacterQuery) WithRecords(opts ...func(*RecordQuery)) *CharacterQuery {
	query := (&RecordClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CharacterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Character, error) {
	...
	if query := cq.withProducts; query != nil {
		if err := cq.loadProducts(ctx, query, nodes,
			func(n *Character) { n.Edges.Products = []*Product{} },
			func(n *Character, e *Product) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withRecords; query != nil {
		if err := cq.loadRecords(ctx, query, nodes,
			func(n *Character) { n.Edges.Records = []*Record{} },
			func(n *Character, e *Record) { n.Edges.Records = append(n.Edges.Records, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CharacterQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Character, init func(*Character), assign func(*Character, *Product)) error {
	...
	query.Where(predicate.Product(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(character.ProductsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.character_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "character_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (cq *CharacterQuery) loadRecords(ctx context.Context, query *RecordQuery, nodes []*Character, init func(*Character), assign func(*Character, *Record)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Character)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
