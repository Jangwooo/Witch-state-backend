...
func (elq *EventLogQuery) QueryUser() *UserQuery {
...
func (elq *EventLogQuery) WithUser(opts ...func(*UserQuery)) *EventLogQuery {
	query := (&UserClient{config: elq.config}).Query()
...
//		GroupBy(eventlog.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.EventLog.Query().
//		Select(eventlog.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (elq *EventLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*EventLog, error) {
	...
	if query := elq.withUser; query != nil {
		if err := elq.loadUser(ctx, query, nodes, nil,
			func(n *EventLog, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (elq *EventLogQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*EventLog, init func(*EventLog), assign func(*EventLog, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*EventLog)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (elq *EventLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := elq.ctx.Fields; len(fields) > 0 {
		...
		if elq.withUser != nil {
			_spec.Node.AddColumnOnce(eventlog.FieldUserID)
		}
	}
