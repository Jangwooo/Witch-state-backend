...
func (mq *MusicQuery) QueryStages() *StageQuery {
	query := (&StageClient{config: mq.config}).Query()
...
func (mq *MusicQuery) QueryRecords() *RecordQuery {
...
func (mq *MusicQuery) Clone() *MusicQuery {
	...
	return &MusicQuery{
		...
		withStages:  mq.withStages.Clone(),
...
func (mq *MusicQuery) WithStages(opts ...func(*StageQuery)) *MusicQuery {
	query := (&StageClient{config: mq.config}).Query()
...
func (mq *MusicQuery) WithRecords(opts ...func(*RecordQuery)) *MusicQuery {
	query := (&RecordClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MusicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Music, error) {
	...
	if query := mq.withStages; query != nil {
		if err := mq.loadStages(ctx, query, nodes,
			func(n *Music) { n.Edges.Stages = []*Stage{} },
			func(n *Music, e *Stage) { n.Edges.Stages = append(n.Edges.Stages, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withRecords; query != nil {
		if err := mq.loadRecords(ctx, query, nodes,
			func(n *Music) { n.Edges.Records = []*Record{} },
			func(n *Music, e *Record) { n.Edges.Records = append(n.Edges.Records, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MusicQuery) loadStages(ctx context.Context, query *StageQuery, nodes []*Music, init func(*Music), assign func(*Music, *Stage)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(stage.FieldMusicID)
	}
	query.Where(predicate.Stage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(music.StagesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "music_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (mq *MusicQuery) loadRecords(ctx context.Context, query *RecordQuery, nodes []*Music, init func(*Music), assign func(*Music, *Record)) error {
	...
	query.Where(predicate.Record(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(music.RecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
