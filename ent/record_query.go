...
func (rq *RecordQuery) QueryMusic() *MusicQuery {
	query := (&MusicClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RecordQuery) QueryStage() *StageQuery {
	query := (&StageClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RecordQuery) WithMusic(opts ...func(*MusicQuery)) *RecordQuery {
	query := (&MusicClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RecordQuery) WithStage(opts ...func(*StageQuery)) *RecordQuery {
	query := (&StageClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RecordQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Record, error) {
	...
	if query := rq.withMusic; query != nil {
		if err := rq.loadMusic(ctx, query, nodes, nil,
			func(n *Record, e *Music) { n.Edges.Music = e }); err != nil {
			return nil, err
		}
	}
...
func (rq *RecordQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withMusic != nil {
			_spec.Node.AddColumnOnce(record.FieldMusicID)
		}
		if rq.withStage != nil {
