...
func (uq *UserQuery) QueryPurchasedProducts() *ProductQuery {
...
func (uq *UserQuery) QueryUserPurchases() *UserPurchaseQuery {
	query := (&UserPurchaseClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithPurchasedProducts(opts ...func(*ProductQuery)) *UserQuery {
	query := (&ProductClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withPurchasedProducts = query
	return uq
}
...
func (uq *UserQuery) WithUserPurchases(opts ...func(*UserPurchaseQuery)) *UserQuery {
	query := (&UserPurchaseClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserPurchases = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPurchasedProducts; query != nil {
		if err := uq.loadPurchasedProducts(ctx, query, nodes,
			func(n *User) { n.Edges.PurchasedProducts = []*Product{} },
			func(n *User, e *Product) { n.Edges.PurchasedProducts = append(n.Edges.PurchasedProducts, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withUserPurchases; query != nil {
		if err := uq.loadUserPurchases(ctx, query, nodes,
			func(n *User) { n.Edges.UserPurchases = []*UserPurchase{} },
			func(n *User, e *UserPurchase) { n.Edges.UserPurchases = append(n.Edges.UserPurchases, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPurchasedProducts(ctx context.Context, query *ProductQuery, nodes []*User, init func(*User), assign func(*User, *Product)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.PurchasedProductsTable)
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(user.PurchasedProductsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.PurchasedProductsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.PurchasedProductsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "purchased_products" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadUserPurchases(ctx context.Context, query *UserPurchaseQuery, nodes []*User, init func(*User), assign func(*User, *UserPurchase)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userpurchase.FieldUserID)
	}
	query.Where(predicate.UserPurchase(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserPurchasesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
